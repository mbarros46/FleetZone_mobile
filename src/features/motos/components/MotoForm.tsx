...
interface MotoFormProps {
  onSubmit: (data: MotoForm) => Promise<void>;
  loading?: boolean;
}

export function MotoForm({ onSubmit, loading = false }: MotoFormProps) {
  ...
  const handleSubmitForm = async (data: MotoForm) => {
    try {
      await onSubmit(data);
      reset();
      Alert.alert('Sucesso', 'Moto cadastrada com sucesso!');
    } catch (error: any) {
      Alert.alert('Erro', error.message || 'Erro ao cadastrar moto');
    }
  };
